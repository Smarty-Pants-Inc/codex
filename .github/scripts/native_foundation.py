...
def initialize():
    ...
    need(
        os.environ["GITHUB_REF"] == "refs/heads/ci/codex-native-foundation-5406-1",
        "wrong ref",
    )
    need(
    ...
    save(
        EVIDENCE / "source.json",
        {
            ...
            "scope": "SCOPED_NATIVE_FOUNDATION",
...
# Producer-side early scope check copied from route2. CI independently receives raw
# discovery/JUnit using its trusted receiver; this function awards no acceptance.
def foundation_roster(full, selected, spec):
    group = spec["id"]
    need(
        group in ("core-all", "core-lib", "app-all", "protocol-all"),
    ...
    if group == "protocol-all":
        need(
            package == "codex-protocol" and not spec["prefixes"],
    ...
    # Derive scope BEFORE excluding ignored/mismatched rows.
    expected = {
        key
        for key in all_cases
        if group == "protocol-all" or key[1].startswith(tuple(spec["prefixes"]))
    }
...
def run():
    failed = False
    ready = {}
    # Preserve all full native discoveries before issuing any selected discovery.
