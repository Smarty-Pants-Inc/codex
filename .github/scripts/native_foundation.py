...
def initialize():
    ...
    need(
        os.environ["GITHUB_REF"] == "refs/heads/ci/codex-native-foundation-5406-4",
        "wrong ref",
    )
    need(
...
def foundation_roster(full, selected, spec):
    group = spec["id"]
    need(
        group in ("core-all", "core-lib", "app-all", "protocol-all", "tui-all"),
...
def run():
    failed = False
    ready = {}
    # Preserve every full native discovery before issuing any selected discovery.
