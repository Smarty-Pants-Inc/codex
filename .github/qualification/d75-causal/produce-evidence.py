...
allowed = {"custody.json", "phases.json", "source-locks.json", "environment.json", "native-executables.json", "lane-exit.txt", "external-lane-exit.txt", "elapsed-seconds.txt", "failure.txt", "wrapper.stdout", "wrapper.stderr", "members.json", "coverage.json", "rusty-v8-paths.txt", "rusty-v8.sha256"}
...
assert custody["workflow_ref"] == "Smarty-Pants-Inc/codex/.github/workflows/smarty-ci.yml@refs/heads/qualify/codex-d75-v8-remediation-1"
...
if (out / "rusty-v8-paths.txt").exists() or (out / "rusty-v8.sha256").exists():
    assert (out / "rusty-v8-paths.txt").is_file() and (out / "rusty-v8.sha256").is_file()
    paths = {}
    for line in (out / "rusty-v8-paths.txt").read_text().splitlines():
        key, value = line.split("=", 1)
        assert key in {"archive", "binding", "checksums"} and key not in paths
        paths[key] = Path(value)
    assert set(paths) == {"archive", "binding", "checksums"}
    assert paths["archive"].name == "librusty_v8_ptrcomp_sandbox_release_x86_64-unknown-linux-gnu.a.gz"
    assert paths["binding"].name == "src_binding_ptrcomp_sandbox_release_x86_64-unknown-linux-gnu.rs"
    assert paths["checksums"].name == "rusty_v8_ptrcomp_sandbox_release_x86_64-unknown-linux-gnu.sha256"
    expected_v8 = {
        paths["archive"].name: "a35c75d1f26e6a983885a45b33490a4ebe54f05050568b32b89cfb421b30b583",
        paths["binding"].name: "7727826ae479bdb645e807239fb12d1f8e2e23de7a6cf16f5ee592690d1d8506",
        paths["checksums"].name: "6774b42c9424c098c72a805c08d4e94be17c591cf02b1dc2633060255a8a61be",
    }
    observed = {}
    for line in (out / "rusty-v8.sha256").read_text().splitlines():
        digest, name = line.split(None, 1)
        observed[Path(name).name] = digest
    assert observed == expected_v8
    assert not any(p.name.startswith(("librusty_v8_", "src_binding_")) and p.name not in {"rusty-v8-paths.txt", "rusty-v8.sha256"} for p in out.iterdir())
