...
"#;
    ...
    // Sampler completion and approval decisions record metrics on different tasks, so only the
    // tool-call decision sequence has a stable order.
    ...
    assert_eq!(
        samples
            .iter()
            .filter(|sample| decision_sequence.contains(sample))
            .cloned()
            .collect::<Vec<_>>(),
        decision_sequence.to_vec()
    );
