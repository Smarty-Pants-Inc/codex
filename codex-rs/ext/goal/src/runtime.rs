...
impl GoalRuntimeHandle {
    ...
    pub(crate) async fn continue_if_idle(&self) -> Result<(), String> {
        ...
        let start_options = thread
            ...
            .unwrap_or_default();
        let input = continuation_developer_input(
        ...
        match submission {
            // The queue may start a queued user turn instead, so the continuation is identified
            // by its idle-turn source when the turn starts (see the goal extension).
