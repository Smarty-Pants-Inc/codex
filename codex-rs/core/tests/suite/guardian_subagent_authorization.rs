...
async fn guardian_subagent_review_preserves_late_root_user_authorization(
    ...
    // Injected user-role history is rejected, so exercise these exclusions and retained
    // instructions through real direct input.
    let mut root_user_texts = Vec::new();
    if !matches!(root_context, RootContext::RetainedAtMessageLimit) {
        // Direct input can look like these synthetic messages.
        root_user_texts.extend([
            format!(
    ...
    if matches!(root_context, RootContext::RetainedAtMessageLimit) {
        ...
        root_user_texts.extend((0..6).map(|index| {
            if index == 0 {
    ...
    for text in root_user_texts {
        let expected_text = text.clone();
        let direct_turn = mount_sse_once_match(
            &server,
            move |request: &wiremock::Request| {
            ...
            sse(vec![ev_completed("root-direct-input-response")]),
        )
        .await;
