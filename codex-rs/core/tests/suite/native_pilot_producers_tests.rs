...
-> anyhow::Result<()> {
    ...
    for request in [
        TurnInputRequest::user_input(vec![codex_protocol::user_input::UserInput::Text {
            text: "must not bypass pilot admission as human input".into(),
        ...
        let error = test
            .codex
            .start_pilot_turn(owner, "fixture-scope".into(), nonce, request)
            .await
            .unwrap_err();
        assert!(matches!(
            error.details(),
    ...
    let result = tokio::time::timeout(Duration::from_secs(/*secs*/ 10), async {
        loop {
            let result = test
                .codex
                .start_pilot_turn(owner, "fixture-scope".into(), next_request, input.clone())
                .await?;
            // Core emits completion before it drops the active reservation. Only
            // a definite pre-admission NotIdle may be retried, never uncertainty.
            if result
