...
pub(crate) async fn prepare_response_items_with_removed_user_content(
    ...
    for mut item in std::mem::take(items) {
        ...
        if let Some(notices) = user_notices.filter(|notices| !notices.is_empty()) {
...
async fn prepare_message_content(
    ...
    for item in items {
        match item.content_mut() {
            ContentItem::InputImage { image, detail } => {
                if let Err(error) =
                    prepare_image(image, detail, origin, metadata, mode, image_store).await
                {
                    warn!(%error, "failed to prepare message image");
