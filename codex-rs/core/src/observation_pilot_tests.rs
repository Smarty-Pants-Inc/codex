...
impl Fixture {
    ...
    fn guard(&self, request_id: Uuid) -> TurnStartGuard {
        self.slot
...
fn missing_wrong_owner_scope_permission_and_replay_are_rejected() {
    ...
    assert!(
        !fixture
            .guard(nonce)
            .try_commit(&fixture.issuer.claims.thread_id, "turn")
    );
    assert_eq!(
...
fn commit_cooldown_native_identity_finite_spend_and_expiry_are_owner_atomic() {
    ...
    assert!(!guard.try_commit(&ThreadId::new(), "wrong-thread"));
    assert!(guard.try_commit(&thread, "turn-one"));
    assert!(!guard.clone().try_commit(&thread, "replay"));
    let second_nonce = Uuid::new_v4();
    let second = fixture.guard(second_nonce);
    assert!(!second.try_commit(&thread, "turn-two"));
...
fn auth_first_generation_failure_permanently_fences_auth_checks_and_admission() {
    ...
    assert!(
        !fixture
            .guard(Uuid::new_v4())
            .try_commit(&fixture.issuer.claims.thread_id, "never-admitted")
    );
}
...
fn actual_attempts_require_qualification_keep_unknown_spend_and_bind_late_usage() {
    ...
    assert!(
        fixture
            .guard(nonce)
            .try_commit(&fixture.issuer.claims.thread_id, "native-turn")
    );
