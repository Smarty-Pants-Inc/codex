...
pub(crate) use self::inject::validate_live_response_items;
...
impl Session {
    ...
    async fn apply_rollout_reconstruction(
        ...
        // metadata on its original prepared response item. Replay must not upload or migrate
        // recorded history: the inline store returns prepared inline bytes, while existing file
        // references bypass preparation and remain unchanged.
    ...
    pub(crate) fn response_items_from_user_input(
        ...
        self.response_items_from_user_input_with_image_positions(input, &mut HashMap::new())
    }

    /// Splits direct input into its user message and any generated developer notices.
    /// `user_image_content_indices` maps image inputs to their user-message content slots.
    fn response_items_from_user_input_with_image_positions(
        &self,
        input: Vec<UserInput>,
    ...
    /// Converts input into one user-role message, for callers that frame generated content
    /// themselves.
    ...
    pub(crate) async fn record_user_prompt_and_emit_turn_item(
        ...
        // UI-only `text_elements` are preserved. Generated notices follow as developer items.
        let mut user_image_content_indices = HashMap::new();
        let response_items = self.response_items_from_user_input_with_image_positions(
            input.to_vec(),
        ...
        let envelopes = response_items
            .into_iter()
            .map(|item| {
                // The acceptance order belongs to the direct user message, not generated media.
                let metadata = if item.is_user_message() {
                    acceptance_metadata.take()
                } else {
                    None
                };
