...
async fn direct_user_replaces_taskless_reservation_and_stale_auto_start_fails() {
    ...
    *session.active_turn.lock().await = Some(reserved_turn);
    ...
    let direct_turn_context = session
        .new_turn_with_default_settings("direct-turn".to_string(), Default::default())
        .await;
    session
        .start_task(
    ...
    let automatic_turn_context = session
        .new_turn_with_default_settings("stale-automatic-turn".to_string(), Default::default())
        .await;
...
async fn start_only_rejects_current_plan_before_validating_settings() {
    ...
    let submission = handle(
        &session,
        TurnInputRequest::new(SubmittedTurnInput::ResponseItem(developer_message(
