...
async fn root_suspension_rechecks_after_in_flight_child_admission() {
    ...
    let runtime = harness.control.runtime.clone();
...
async fn persist_thread_for_tree_resume(thread: &Arc<CodexThread>, message: &str) {
    ...
    *thread.session.active_turn.lock().await = None;
    let _ = thread.session.input_queue.drain_mailbox_input_items().await;
    record_direct_user_message(thread, message).await;
    thread
        .session
...
async fn record_direct_user_message(thread: &Arc<CodexThread>, message: &str) {
    let turn_context = thread.session.new_default_turn().await;
    thread
        .session
...
async fn ensure_v2_agent_loaded_reloads_registered_unloaded_agent() {
    Box::pin(check_v2_agent_reload(V2ReloadRoute::Sender)).await;
}

#[tokio::test]
async fn ensure_v2_child_loaded_preserves_evicted_parent_authority() {
    Box::pin(check_v2_agent_reload(V2ReloadRoute::NestedParent)).await;
}
...
async fn spawn_agent_fork_from_paginated_parent_uses_model_context_prefix() {
    ...
    record_direct_user_message(&parent_thread, "paginated parent context").await;
...
async fn spawn_agent_without_fork_from_paginated_parent_stays_fresh_and_paginated() {
    let harness = AgentControlHarness::new().await;
    let (parent_thread_id, parent_thread) = harness.start_paginated_thread().await;
    record_direct_user_message(&parent_thread, "parent-only context").await;
...
async fn spawn_agent_can_fork_parent_thread_history_with_sanitized_items() {
    ...
    let mut expected_final_answer = parent_thread
        .session
        .clone_history()
        .await
        .raw_items()
    ...
    if let (
        ...
        ResponseItem::Message {
            internal_chat_message_metadata_passthrough: Some(actual_metadata),
            ..
        },
    ) = (&mut expected_final_answer, &history_items[2])
    {
        expected_metadata.turn_id = actual_metadata.turn_id.clone();
    }
...
async fn spawn_agent_fork_strips_parent_usage_hints_from_compacted_history() {
    ...
    let inherited_context = codex_history::RetainedContext::default();
