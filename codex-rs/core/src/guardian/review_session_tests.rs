...
async fn cancellation_only_admits_evidence_recorded_before_the_terminal_event() {
    use codex_guardian_reviewer::ReviewerRuntime;

    for record_input in [false, true] {
        let (review_session, _events, _submissions) = test_review_session().await;
        ...
        let recorded = Event {
            ...
            msg: EventMsg::RawResponseItem(codex_protocol::protocol::RawResponseItemEvent { item }),
        };
        // The reviewer drain admits each event of the interrupted turn up to its terminal event.
        if record_input {
            review_session.admit_context(&recorded).await;
            review_session.admit_context(&recorded).await;
        }
        review_session
            .admit_context(&turn_aborted_event("child-turn"))
            .await;
