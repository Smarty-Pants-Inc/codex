...
-> anyhow::Result<()> {
    ...
    let guardian_developer_text = request.message_input_texts("developer").concat();
    assert!(guardian_developer_text.contains(">>> TRANSCRIPT START\n"));
    assert!(
        guardian_developer_text.contains("The Codex agent has requested the following action:\n")
    );
    assert!(guardian_developer_text.contains(&format!(
