...
/// Keeps quoted evidence from forging the `>>>` section boundaries that frame it.
pub(crate) fn escape_section_markers(text: &str) -> String {
    text.replace(">>>", "> > >")
}
    ...
    fn contribute(&self, input: &SectionInput<'_>) -> Result<Option<ContextSection>, SectionError> {
        ...
        items.extend(
            input
                ...
                .map(|text| escape_section_markers(&text)),
        );
