...
pub fn render_review_evidence(review: ReviewEvidence<'_>) -> RenderedReviewEvidence {
    ...
    // Reviewed actions and rationales are evidence; they must not close the evidence sections.
    let body = truncate(
        "sync_review_body",
        crate::authorization::escape_section_markers(&body),
        MAX_REVIEW_BODY_TOKENS,
    );
