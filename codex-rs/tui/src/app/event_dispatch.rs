    ...
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::OracleAttachThread {
                ...
                match self
                    .attach_oracle_thread_binding(conversation_id.clone(), url.clone(), Some(title))
                    .await
                {
                    Ok(binding) => {
                        ...
                        match binding {
                            ...
                            OracleAttachThreadBinding::AttachedRemote {
                                ...
                                self.chat_widget.add_to_history(oracle_history_cell(
                                    "Oracle",
                                    &format!(
                                        "Attached Oracle thread {thread_id} to existing conversation {conversation_id}."
                                    ),
                                ));
                            }
                        }
                    ...
                    Err(err) => self.chat_widget.add_error_message(format!(
                        "Failed to attach Oracle thread to existing conversation: {err}"
                    )),
                }
            }
