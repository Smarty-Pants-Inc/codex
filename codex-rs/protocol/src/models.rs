...
/// Converts user input into content items, marking host-generated framing and placeholders and
/// recording the originating input index of each item.
fn converted_user_input_content(
    items: Vec<UserInput>,
    local_image_preparation: LocalImagePreparation,
) -> Vec<(ContentItem, bool, usize)> {
    ...
    items
        ...
        .flat_map(|(input_index, input)| {
            ...
            items
                .into_iter()
                .map(move |(item, generated)| (item, generated, input_index))
        })
        .collect()
}
...
impl ResponseInputItem {
    ...
    pub fn from_user_input(
        ...
        for (item, _generated, input_index) in
            converted_user_input_content(items, local_image_preparation)
        {
            ...
            if matches!(item, ContentItem::InputImage { .. }) {
                user_image_content_indices.insert(input_index, content.len());
            }
            content.push(item);
        }
        Self::Message {
    ...
    /// `user_image_content_indices` maps each image input to its slot in the user message.
    pub fn from_user_input_with_generated_content(
        ...
        for (item, generated, input_index) in
            converted_user_input_content(items, local_image_preparation)
        {
            if generated {
                ...
                if matches!(item, ContentItem::InputImage { .. }) {
                    user_image_content_indices.insert(input_index, user_content.len());
                }
    ...
    pub fn from_developer_input(
        ...
        let mut input = Self::from_user_input(items, local_image_preparation, &mut HashMap::new());
